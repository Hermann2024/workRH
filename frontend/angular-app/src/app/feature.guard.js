...
export const featureGuard = (feature) => {
    return () => {
        const router = inject(Router);
        const api = inject(WorkRhApiService);
        return api.checkFeature(feature).pipe(map((response) => response.allowed ? true : router.parseUrl('/pricing')), catchError(() => of(router.parseUrl('/pricing'))));
    };
};
